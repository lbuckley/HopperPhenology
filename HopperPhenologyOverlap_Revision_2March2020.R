...
param.mu= fit.df[which(fit.df$param=="mu"),] #mu scale sig
param.mu= na.omit(param.mu)
param.mu.sd= fit.df[which(fit.df$param=="mu.sd"),]
param.mu.sd= na.omit(param.mu.sd)
#match
param.mu$spsiyr= paste(param.mu$siteyear, param.mu$species, sep="")
param.mu.sd$spsiyr= paste(param.mu.sd$siteyear, param.mu.sd$species, sep="")

match1= match(param.mu$spsiyr, param.mu.sd$spsiyr)
...
#----
#sig
param.sig= fit.df[which(fit.df$param=="sig"),] #sig scale sig
param.sig= na.omit(param.sig)
param.sig.sd= fit.df[which(fit.df$param=="sig.sd"),]
param.sig.sd= na.omit(param.sig.sd)
#match
param.sig$spsiyr= paste(param.sig$siteyear, param.sig$species, sep="")
param.sig.sd$spsiyr= paste(param.sig.sd$siteyear, param.sig.sd$species, sep="")

match1= match(param.sig$spsiyr, param.sig.sd$spsiyr)
...
param.scale= fit.df[which(fit.df$param=="scale"),] #scale scale sig
param.scale= na.omit(param.scale)
param.scale.sd= fit.df[which(fit.df$param=="scale.sd"),]
param.scale.sd= na.omit(param.scale.sd)
#match
param.scale$spsiyr= paste(param.scale$siteyear, param.scale$species, sep="")
param.scale.sd$spsiyr= paste(param.scale.sd$siteyear, param.scale.sd$species, sep="")

match1= match(param.scale$spsiyr, param.scale.sd$spsiyr)
...
#-------------------------
#Variance across mean / peak phenology

param.mu= fit.df[which(fit.df$param=="mu"),] #mu scale sig
param.mu= na.omit(param.mu)

mu.var= ddply(param.mu, c("site", "year","period","siteyear"), summarise,
